...
export function HistorySidebar({
    ...
    return (
        <>
            ...
            {/* Sidebar main panel */}
            <div className={`absolute top-0 left-0 w-72 h-full bg-white/95 dark:bg-black/90 backdrop-blur-md border-r border-stone-200 dark:border-white/10 p-5 z-40 text-stone-800 dark:text-white shadow-2xl transition-transform duration-300 ease-in-out pt-[calc(1.25rem+env(safe-area-inset-top))] pl-[calc(1.25rem+env(safe-area-inset-left))] flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="flex-1 flex flex-col gap-5 overflow-hidden min-h-0">
                    ...
                    <div className="shrink-0 flex flex-col max-h-[30%] min-h-[100px] overflow-hidden">
                        <h3 className="text-xs font-bold uppercase tracking-wider text-stone-500 dark:text-gray-400 border-b border-stone-200 dark:border-white/10 pb-1 mb-2 font-sans shrink-0">
                            Agent Memory
                        </h3>
                        {Object.keys(dictOfVars).length === 0 ? (
                            <p className="text-stone-400 dark:text-gray-500 text-[11px] leading-relaxed font-sans pr-1">
                            ...
                            <div className="flex-col gap-2 overflow-y-auto pr-1 flex scrollbar-thin">
                                {Object.entries(dictOfVars).map(([key, value]) => (
                                    <div key={key} className="flex justify-between items-center bg-stone-50 dark:bg-white/5 p-2 rounded-lg border border-stone-200 dark:border-white/10 shadow-inner py-1.5">
                                        <span className="text-sm font-mono text-purple-600 dark:text-purple-400 font-bold">{key}</span>
                                        <span className="text-sm font-mono text-green-600 dark:text-green-400 font-bold">= {value}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        ...
                        {history.length === 0 ? (
                            <p className="text-stone-400 dark:text-gray-500 text-[11px] leading-relaxed font-sans">
                            ...
                            <div className="flex-1 flex flex-col overflow-hidden">
                                    ...
                                    {history.map((entry) => {
                                        ...
                                        return (
                                            <div 
                                                ...
                                                className="cursor-pointer group flex gap-2.5 items-center bg-stone-50 dark:bg-white/5 border border-stone-200 dark:border-white/10 hover:border-stone-300 dark:hover:border-white/20 p-2 rounded-xl transition-all shadow hover:bg-stone-100/50 dark:hover:bg-white/[0.08]"
                                            >
                                                <div className="w-14 h-10 bg-stone-200 dark:bg-black rounded-lg overflow-hidden border border-stone-300 dark:border-white/15 flex-shrink-0 flex items-center justify-center">
                                                    {entry.canvasThumbnail ? (
                                                        ...
                                                        <span className="text-[9px] text-stone-500 dark:text-gray-600 font-sans">Empty</span>
                                                    )}
                                                </div>
                                                <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                                                    <span className="text-xs text-stone-850 dark:text-gray-200 font-bold truncate font-sans">
                                                        {summary}
                                                    </span>
                                                    <span className="text-[10px] text-stone-400 dark:text-gray-500 font-sans">
                                                        {dateStr}
                                                    </span>
                                                </div>
                                                <button
                                                    ...
                                                    className="cursor-pointer text-stone-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 p-1 rounded transition-colors shrink-0"
                                ...
                                <div className="shrink-0 mt-2">
                                    {confirmClear ? (
                                        <div className="flex gap-2 items-center bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-500/20 p-2 rounded-xl animate-in fade-in zoom-in-95">
                                            <span className="text-[10px] text-red-600 dark:text-red-400 font-bold font-sans">Clear all history?</span>
                                            <button
                                                onClick={() => {
                                                    onClearHistory();
