...
def analyze_image(img: Image, dict_of_vars: dict, is_retry: bool = False):
    ...
    prompt = (
        ...
        f"3. 'steps': A list of discrete solution steps if solving requires multiple steps, else null. Break the solution into 2-6 discrete steps representing one meaningful transformation/reasoning move. Return each step as a dict: {{'order': int, 'description': str, 'expression': Optional[str]}} where 'expression' holds any LaTeX/math for that specific step. "
        f"Following are the cases: "
        f"1. Simple mathematical expressions like 2 + 2, 3 * 4, 5 / 6, 7 - 8, etc.: In this case, solve and return the answer in the format of a LIST OF ONE DICT [{{'expr': given expression, 'result': calculated answer, 'type': 'math', 'thought_process': '...', 'confidence_score': 99, 'steps': null}}]. "
        f"2. Set of Equations like x^2 + 2x + 1 = 0, 3y + 4x = 0, 5x^2 + 6y + 7 = 12, etc.: In this case, solve for the given variable, and the format should be a COMMA SEPARATED LIST OF DICTS, with dict 1 as {{'expr': 'x', 'result': 2, 'assign': True, 'type': 'math', 'thought_process': '...', 'confidence_score': 95, 'steps': [{{'order': 1, 'description': '...', 'expression': '...'}}, ...]}} and dict 2 as {{'expr': 'y', 'result': 5, 'assign': True, 'type': 'math', 'thought_process': '...', 'confidence_score': 95, 'steps': [...]}}. Include as many dicts as there are variables. "
        f"3. Assigning values to variables like x = 4, y = 5, z = 6, etc.: In this case, assign values to variables and return another key in the dict called {{'assign': True}}, keeping the variable as 'expr' and the value as 'result' and include {{'type': 'math', 'thought_process': '...', 'confidence_score': 100, 'steps': null}}. RETURN AS A LIST OF DICTS. "
