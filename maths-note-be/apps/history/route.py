...
async def get_history_endpoint(request: Request, user_id: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch user history.")


@router.post("", response_model=dict[str, Any])
@limiter.limit("10/minute")
async def save_entry_endpoint(
    request: Request, payload: SingleEntrySaveRequest, user_id: str = Depends(get_current_user)
):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save history entry.")


@router.post("/sync", response_model=dict[str, Any])
@limiter.limit("10/minute")
async def sync_history_endpoint(
    request: Request, payload: SyncHistoryRequest, user_id: str = Depends(get_current_user)
):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to sync history entries.")
...
async def purge_history_endpoint(request: Request, user_id: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to purge user history.")
...
async def delete_entry_endpoint(request: Request, entry_id: str, user_id: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete history entry.")
