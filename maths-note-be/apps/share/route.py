...
async def get_share_endpoint(share_id: str):
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve share data.")

    if not share:
        raise HTTPException(status_code=404, detail="Share link has expired or does not exist.")

    return {"status": "success", "image": share["image"], "data": share["data"]}
